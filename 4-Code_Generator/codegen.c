...
static int code_index = 0, sym_index = 0, lex_level = 0;
static int procedure_address[500];

static symbol *table;
static lexeme token, *list;
...
void block(int proc_id)
{
	...
	// Use space to track how much space should be allocated for the
	// activation record and variables 
	int space = 3;
	...
	if (wePutJMP)
		code[0].m = code_index * 3;

	// Store the starting point of the current procedure
	if (proc_id != 0)
		procedure_address[proc_id] = code_index * 3;

	// Instruction to allocate space for variables and activation record
	emit(INC, 0, space);
...
void procedure_declaration()
{
	...
	// with CAL later on
	addSymbol(ident.name, proc_id, prockind, 0);
	...
	// Increase the lex_level for everything inside the procedure.
	lex_level++;
	...
	// Decrease lex_level now that we're outside the procedure
	lex_level--;
...
void statement()
{
	...
	case identsym:
		...
		// lex_level and the address of the variable
		emit(STO, lex_level - sym->level, sym->addr);
		break;
	...
	case callsym:
		...
		// lex_level and the address of the procedure
		emit(CAL, lex_level - sym->level, procedure_address[sym->val]);
		break;
...
void factor()
{
	if (token.type == identsym)
	{
		...
		if (sym->kind == constkind)
			...
			emit(LOD, lex_level - sym->level, sym->addr);
...
void addSymbol(char *name, int val, int kind, int address)
{
	table[sym_index] = (symbol){kind, "", val, lex_level, address};
...
symbol *fetchSymbol(char *name, int options)
{
	for (int i = sym_index - 1; i >= 0; i--)
	{
		...
		if ((options & op_sameScope) && table[i].level != lex_level)
			break;
